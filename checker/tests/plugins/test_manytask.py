...
class TestManytaskPlugin:
    ...
    def test_date_without_timezone_throws_warning(self) -> None:
        ...
        with Mocker() as mocker:
            mocker.post(f"{self.REPORT_URL}", status_code=HTTP_STATUS_OK, text='{"score": 1.0}')
    ...
    @pytest.mark.parametrize(
        ...
        [
            (HTTP_STATUS_OK, "Success", None),
            (HTTP_STATUS_REQUEST_TIMEOUT, "Request Timeout", PluginExecutionFailed),
            (HTTP_STATUS_SERVICE_UNAVAILABLE, "Service Unavailable", PluginExecutionFailed),
        ],
    )
