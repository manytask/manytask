...
def sample_correct_pipeline() -> list[PipelineStageConfig]:
    return [
        ...
        PipelineStageConfig(
            ...
            run="fail",
            run_if=f"${{{{ outputs.score_stage.percentage > {SCORE_70_PERCENT} }}}}",
        ),
        PipelineStageConfig(
...
class TestSampleFixtures:
    def test_plugins(self, sample_plugins: dict[str, Type[PluginABC]]) -> None:
        ...
        assert result.percentage == SCORE_100_PERCENT
        assert result.output == "Hello"

        plugin = sample_plugins["score"]()
        plugin.validate({"score": SCORE_20_PERCENT})
        result = plugin.run({"score": SCORE_20_PERCENT})
        assert result.percentage == SCORE_20_PERCENT
...
class TestPipelineRunnerValidation:
    ...
    def test_invalid_run_if(
        ...
        with pytest.raises(BadConfig):
            pipeline_runner.validate({"score": SCORE_50_PERCENT}, validate_placeholders=True)
