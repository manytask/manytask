...
class ManytaskDeadlinesConfig(CustomBaseModel):
    ...
    window: Optional[int] = (
        None  # interpolation window (in days) used for ManytaskDeadlinesType.INTERPOLATE
    )
    ...
    def check_valid_deadline(self) -> "ManytaskDeadlinesConfig":
        if (
            ...
            and self.deadlines != ManytaskDeadlinesType.INTERPOLATE
        ):
            raise ValueError(
                "window can be applied only with interpolate deadline type"
            )
    ...
    def max_score(
        ...
        return sum(
            ...
            for task in self.get_tasks(
                enabled=True, started=started, is_bonus=False, now=now
            )
        )
