...
def validate_json_request(model_class: type[BaseModel]) -> tuple[BaseModel | None, ResponseReturnValue | None]:
    ...
    try:
        ...
        return None, (jsonify(ErrorResponse(error=f"Invalid request data: {exc.errors()}").model_dump()), HTTPStatus.BAD_REQUEST)
...
def requires_namespace_admin(return_404_if_not_found: bool = False) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
            ...
            if not is_instance_admin:
                try:
                    ...
                    if user_role != "namespace_admin":
                        ...
                        return jsonify(ErrorResponse(error="Only Instance Admin or Namespace Admin can perform this action").model_dump()), HTTPStatus.FORBIDDEN
...
def create_namespace() -> ResponseReturnValue:
    ...
    if not storage_api.check_if_instance_admin(username):
        return jsonify(ErrorResponse(error="Only Instance Admin can create namespaces").model_dump()), HTTPStatus.FORBIDDEN
...
def add_user_to_namespace(namespace_id: int, namespace: Any, validated_data: AddUserToNamespaceRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        try:
            ...
            return jsonify(ErrorResponse(error="Failed to add user to GitLab group").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
        ...
        try:
            ...
            return jsonify(ErrorResponse(error="User already has a role in this namespace").model_dump()), HTTPStatus.CONFLICT
...
def get_namespace_users(namespace_id: int, namespace: Any) -> ResponseReturnValue:
    ...
    try:
        ...
        users = [
            NamespaceUserItem(user_id=user_id, role=user_role)
            for user_id, user_role in users_list
        ]
        ...
        logger.error("Error fetching users for namespace id=%s by user=%s: %s", namespace_id, username, str(e), exc_info=True)
        return jsonify(ErrorResponse(error="Internal server error").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
...
def create_course_api(validated_data: CreateCourseRequest) -> ResponseReturnValue:
    ...
    try:
        ...
        try:
            ...
            return jsonify(ErrorResponse(error="Namespace not found or access denied").model_dump()), HTTPStatus.NOT_FOUND
        ...
        try:
            ...
            return jsonify(ErrorResponse(error=f"Failed to create course group: {str(e)}").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
        ...
        try:
            ...
            return jsonify(ErrorResponse(error=f"Failed to create public repo: {str(e)}").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
        ...
        try:
            ...
            if course_group_id:
                rms_api.delete_group(course_group_id)
            return jsonify(ErrorResponse(error=f"Failed to create students group: {str(e)}").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
        ...
        try:
            ...
            if course_group_id:
                rms_api.delete_group(course_group_id)
            return jsonify(ErrorResponse(error=f"Failed to create course in database: {str(e)}").model_dump()), HTTPStatus.INTERNAL_SERVER_ERROR
