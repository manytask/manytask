...
def test_task_url_template_with_user_name():
    ...
    template = """
    {% set task_link = task_url_template | replace('$GROUP_NAME', group_name) | replace('$TASK_NAME', task_name) | replace('$USER_NAME', username) %}
    {{ task_link }}
    """
...
def test_ui_config_accepts_user_name_macro():
    ...
    config = ManytaskUiConfig(
        task_url_template="https://gitlab.com/$GROUP_NAME/$USER_NAME/$TASK_NAME"
    )
    assert "$USER_NAME" in config.task_url_template

    # Test valid template with only $USER_NAME
    config = ManytaskUiConfig(
        task_url_template="https://gitlab.com/students/$USER_NAME/tasks"
    )
    assert "$USER_NAME" in config.task_url_template
