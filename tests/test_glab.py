...
def test_create_public_repo(gitlab, mock_gitlab_group, mock_gitlab_project):
    ...
    gitlab_api.create_public_repo(course_group=TEST_GROUP_NAME, course_public_repo=TEST_GROUP_PUBLIC_NAME)
...
def test_create_students_group(gitlab, mock_gitlab_group):
    ...
    gitlab_api.create_students_group(course_students_group=TEST_GROUP_NAME)

    mock_gitlab_instance.groups.create.assert_called_once_with(
        {
            "name": gitlab_api._course_students_group,
            "path": gitlab_api._course_students_group,
            "visibility": "private",
            "lfs_enabled": True,
            "shared_runners_enabled": True,
        }
...
def test_check_project_exists(gitlab, mock_gitlab_student_project, mock_student):
    ...
    exists = gitlab_api.check_project_exists(student=mock_student, course_students_group=TEST_GROUP_NAME)
...
def test_create_project_existing_project(gitlab, mock_student, mock_gitlab_student_project, mock_gitlab_group_member):
    ...
    gitlab_api.create_project(
        student=mock_student, course_students_group=TEST_GROUP_STUDENT_NAME, course_public_repo=TEST_GROUP_PUBLIC_NAME
    )
...
def test_create_project_no_existing_project_creates_fork(
    ...
    gitlab_api.create_project(mock_student)

    mock_gitlab_instance.projects.list.assert_called_with(get_all=True, search=mock_student.username)
    gitlab_api._get_project_by_name.assert_called_with(gitlab_api._course_public_repo)
    gitlab_api._get_group_by_name.assert_called_with(gitlab_api._course_students_group)
...
def test_check_is_course_admin(gitlab):
    ...
    is_admin = gitlab_api._check_is_course_admin(user_id=TEST_USER_ID, course_group=TEST_GROUP_NAME)
...
def test_parse_user_to_student(gitlab, mock_student):
    ...
    student = gitlab_api._parse_user_to_student(
        user=user_dict, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
    )

    assert student == mock_student
    gitlab_api.get_url_for_repo.assert_called_once_with(mock_student.username)
    gitlab_api._check_is_course_admin.assert_called_once_with(TEST_USER_ID)
...
def test_get_student_by_username_found(gitlab, mock_student):
    ...
    result_student = gitlab_api.get_student_by_username(
        username=TEST_USERNAME, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
    )
...
def test_get_student_by_username_not_found(gitlab):
    ...
    with pytest.raises(GitLabApiException, match=f"No students found for username {TEST_USERNAME}"):
        gitlab_api.get_student_by_username(
            username=TEST_USERNAME, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
        )
...
def test_get_student_found(gitlab, mock_user, mock_student):
    gitlab_api, mock_gitlab_instance = gitlab
    user_attrs = {"id": TEST_USER_ID, "username": "test_username", "name": "Test User"}
    ...
    gitlab_api._parse_user_to_student.assert_called_once_with(
        user=user_attrs, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
    )
...
def test_get_authenticated_student_success(mock_get, gitlab, mock_student):
    ...
    student = gitlab_api.get_authenticated_student(
        oauth_token=oauth_token, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
    )

    assert student == mock_student
    mock_get.assert_called_once_with(f"{gitlab_api.base_url}/api/v4/user", headers=headers)
    gitlab_api._parse_user_to_student.assert_called_once_with(
        user=user_data, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
    )
...
def test_get_authenticated_student_failure(mock_get, gitlab):
    ...
    with pytest.raises(HTTPError, match="401 Unauthorized"):
        gitlab_api.get_authenticated_student(
            oauth_token=oauth_token, course_group=TEST_GROUP_NAME, course_students_group=TEST_GROUP_STUDENT_NAME
        )
