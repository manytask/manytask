...
def test_create_namespace_as_instance_admin(client_with_db, session, mock_session_admin):
    ...
    user_on_namespace = (
        session.query(UserOnNamespace)
        .filter_by(namespace_id=namespace.id, user_id=admin_user.id)
        .first()
    )
...
def test_get_namespaces_as_regular_user_without_roles(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_get_namespace_by_id_as_regular_user_with_access(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_get_namespace_by_id_as_regular_user_without_access(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_add_user_to_namespace_as_instance_admin(client_with_db, session, mock_session_admin):
    ...
    app.rms_api.users[regular_user.rms_id] = app.rms_api.get_rms_user_by_id.__func__(
        app.rms_api, regular_user.rms_id
    ) if regular_user.rms_id in app.rms_api.users else None
    ...
    user_on_namespace = (
        session.query(UserOnNamespace)
        .filter_by(user_id=regular_user.id, namespace_id=namespace_id)
        .first()
    )
    assert user_on_namespace is not None
...
def test_add_user_to_namespace_as_program_manager_forbidden(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_get_namespace_users_as_instance_admin(client_with_db, session, mock_session_admin):
    ...
    assert len(data["users"]) == 2  # admin + new_user
...
def test_get_namespace_users_as_namespace_admin(client_with_db, session, mock_session_admin, mock_session_regular):
    ...
    assert "users" in data
    assert len(data["users"]) == 2  # admin + regular_user
...
def test_get_namespace_users_as_program_manager_forbidden(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_remove_user_from_namespace_as_instance_admin(client_with_db, session, mock_session_admin):
    ...
    user_on_namespace = (
        session.query(UserOnNamespace)
        .filter_by(user_id=new_user.id, namespace_id=namespace_id)
        .first()
    )
    assert user_on_namespace is not None
    assert user_on_namespace.role == UserOnNamespaceRole.PROGRAM_MANAGER
...
def test_remove_user_from_namespace_as_namespace_admin(client_with_db, session, mock_session_admin, mock_session_regular):
...
def test_remove_user_from_namespace_as_program_manager_forbidden(client_with_db, session, mock_session_admin, mock_session_regular):
